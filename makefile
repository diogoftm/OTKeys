CC = /usr/bin/gcc
CFLAGS = -g -w -O3 -Wall -Wextra -fPIC -DKMS_URI='"$(KMS_URI)"' -DROOT_CA='"$(ROOT_CA)"' -DSENDER_SAE_CRT='"$(SENDER_SAE_CRT)"' -DSENDER_SAE_KEY='"$(SENDER_SAE_KEY)"' -DRECEIVER_SAE_CRT='"$(RECEIVER_SAE_CRT)"' -DRECEIVER_SAE_KEY='"$(RECEIVER_SAE_KEY)"' -DSENDER_SAE_ID='"$(SENDER_SAE_ID)"' -DRECEIVER_SAE_ID='"$(RECEIVER_SAE_ID)"' -DSENDER_STRICT_ROLE='"$(SENDER_STRICT_ROLE)"' -DRECEIVER_STRICT_ROLE='"$(RECEIVER_STRICT_ROLE)"'
AS = $(CC) $(CFLAGS) -c
